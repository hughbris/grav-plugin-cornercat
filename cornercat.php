...
class CornercatPlugin extends Plugin
{
	...
	private function populateSnippet(): void {
		$extraStyles = '';
		foreach($this->options['styles'] as $prop => $val) {
		...
		if(trim($this->options['position']) == 'left') {
			$svg = <<<EOS
			<svg xmlns="http://www.w3.org/2000/svg" width="80" height="80" viewBox="0 0 250 250" style="fill:{$this->options['fill']}; color:{$this->options['color']}; position: absolute; top: 0; left: 0{$extraStyles}" aria-hidden="true">
...
EOS;
			...
			$svg = <<<EOS
			<svg xmlns="http://www.w3.org/2000/svg" width="80" height="80" viewBox="0 0 250 250" style="fill:{$this->options['fill']}; color:{$this->options['color']}; position: absolute; top: 0; border: 0; right: 0;{$extraStyles}" aria-hidden="true">
